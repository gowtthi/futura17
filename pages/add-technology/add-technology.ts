...
export class AddTechnology {
   ...
   private baseURI               : string  = "http://bitfutura.in/";
   ...
   // for the record data
   createEntry(name, description,answer)
   {
      let body     : string   = "key=create&name=" + name + "&description=" + description +"&answer="+ answer ,
          type     : string   = "application/x-www-form-urlencoded; charset=UTF-8",
          headers  : any      = new Headers({ 'Content-Type': type}),
          options  : any      = new RequestOptions({ headers: headers }),
          url      : any      = this.baseURI + "app/answer.php";
   ...
   // for the record data
   updateEntry(name, description,answer)
   {
      let body       : string = "key=update&name=" + name + "&description=" + description + "&recordID=" + this.recordID+ "&answer="+answer,
          type       : string = "application/x-www-form-urlencoded; charset=UTF-8",
          headers    : any     = new Headers({ 'Content-Type': type}),
          options    : any     = new RequestOptions({ headers: headers }),
          url        : any     = this.baseURI + "app/answer.php";
   ...
   saveEntry()
   {
      ...
      if(this.isEdited)
      {
         this.updateEntry(name, description,answer);
      }
      else
      {
         this.createEntry(name,description,answer);
      }
   }
